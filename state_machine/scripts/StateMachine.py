...
class SceneFlow(smach.State):
    ...
    def execute(self, userdata):
        ...
        if self.dialog < len(self.script):
            ...
            if self.scene_index < len(self.scenes):
              userdata.scene = self.scenes[scene_index]
              self.load_dialog_script()
    ...
    def load_dialog_script(self):
      self.dialog = 0
      with open(dirname(abspath(__file__)) + "/" + self.scenes[self.scene_index] + ".yml", "r") as f:
...
def main():
    ...
    with sm:
        ...
        smach.StateMachine.add('SCENE', SceneFlow(), 
                               transitions={'say_robot_line':'SPEAK', 
                                            'say_ship_line':'SPEAK',
        ...
        smach.StateMachine.add('SPEAK2', Speak(), 
                               transitions={'speech_done':'RESOLVE_QUESTION'})
        
        smach.StateMachine.add('GET_QUESTION',
                           ServiceState('/question_recognition',
                                        SpeechRecognition,
                                        response_slots = ['question']),
                           transitions={'succeeded':'RESOLVE_QUESTION'})
        
        smach.StateMachine.add('RESOLVE_QUESTION', SayResponses(), 
                               transitions={'say_robot_line':'SPEAK2',
